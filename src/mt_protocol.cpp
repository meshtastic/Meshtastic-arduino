...
bool handle_node_info(meshtastic_NodeInfo *nodeInfo) {
  ...
  if (nodeInfo->has_user) {
    node.user_id = (const char *) nodeInfo->user.id;
    node.long_name = (const char *) nodeInfo->user.long_name;
    node.short_name = (const char *) nodeInfo->user.short_name;
    node.macaddr = (const char *) nodeInfo->user.macaddr;
  } else {
    node.user_id = NULL;
    node.long_name = NULL;
...
bool handle_packet(uint32_t now, size_t payload_len) {
  ...
  switch (fromRadio.which_payload_variant) {
    ...
    {
      meshtastic_NodeInfo field = meshtastic_NodeInfo_init_zero;
      pb_decode(&stream, meshtastic_NodeInfo_fields, &field);
      return handle_node_info(&field);
    }
